...
def doctorwise_lab
	...
	test_bookings = TestBooking.find(:all, :conditions=>"created_at BETWEEN '#{from_date}' AND '#{to_date}' AND refferal_doctor LIKE '#{@name.name}' ")
		require 'rubygems'
		  		require 'thinreports'
		  		ThinReports::Report.generate_file('app/views/reports/external.pdf') do
				    ...
				    start_new_page do
		        		...
		        		for test_booking in test_bookings
		        	  				report.page.list(:list).add_row do |row|
		        	    			row.item(:s_no).value s_no
		        			test_booking_child=TestBookingChild.find_by_test_booking_id_and_department(test_booking.id,lab)
		        	    			row.item(:mr_no).value test_booking.mr_no
		        	    			reg=Registration.find_by_mr_no(test_booking.mr_no)
		        	    			row.item(:p_name).value test_booking.patient_name
		        	    			row.item(:age).value reg.age.to_s+"/"+reg.gender
		  		...
		  		ThinReports::Report.generate_file('app/views/reports/test_book.pdf') do
				    ...
				    start_new_page do
		        		org_master_child=OrgMasterChildTable.last
				            ...
				            if(from_date!="" && to_date!="" && patient_type=="OP")
				            	test_bookings=TestBooking.find(:all, :conditions=>"created_at BETWEEN '#{from_date}' AND '#{to_date}'  AND patient_type = 'OP' ")	
						            s_no=1
						        	  	total=0
						        		for test_booking in test_bookings
				        	...
				        	elsif(from_date!="" && to_date!="" && patient_type=="IP")
				        		test_bookings=TestBooking.find(:all, :conditions=>"created_at BETWEEN '#{from_date}' AND '#{to_date}'  AND patient_type = 'IP' ")	
						            s_no=1
