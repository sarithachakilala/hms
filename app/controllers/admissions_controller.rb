  ...
  def create
    ...
    respond_to do |format|
      if @admission.save
		...
		@bedmaster=BedMaster.find_by_id(@admission.bed_id)
		@roommaster=RoomMaster.find_by_id(@bedmaster.room_master_id)
...
def bed_select
 ...
 def ajax_buildings
	...
	if(type=="package_category")
		...
		str=""
		@employee=EmployeeMaster.find(:all, :conditions => "department = '#{mr1}' and org_code = '#{org_code}' OR visible=1")
