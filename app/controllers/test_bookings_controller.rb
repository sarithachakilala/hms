  ...
  def destroy
   ...
   def ajax_function
		...
		if(selected_type=="due")
			...
			if(charge)
				registration=Registration.last(:conditions => "mr_no ='#{params[:mr_no]}' and org_code='#{org_code}'")
				render :text =>charge.service_name+"<division>"+charge.test_name+"<division>"+charge.charge.to_s+"<division>"+charge.service_group+"<division>"+registration.reg_type
			else
				render :text => "error"
			end
