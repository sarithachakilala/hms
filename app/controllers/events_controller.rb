class EventsController < ApplicationController
  ...
  def get_events
	...
	@session = Session.find(session[:id])
	@person = Person.find(@session.person_id)
    @events = Event.find(:all, :conditions => ["(starttime >= '#{Time.at(params['start'].to_i).to_formatted_s(:db)}' and endtime <= '#{Time.at(params['end'].to_i).to_formatted_s(:db)}') and person_id='#{@person.id}'"] )
    events = [] 
