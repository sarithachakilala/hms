...
@session_id=session[:id]
  ...
  def new
	...
	@op=IssuesToOp.last(:select=>"DISTINCT receipt_no",:conditions =>"org_code='#{@org_code}'")
    	if(@op)
		n=(@op.receipt_no.slice!(3..100).to_i+1).to_s
	else
		n=1.to_s
	end
	str="ISS"+n
  ...
  def create
	...
	respond_to do |format|
		if @issues_to_op.save 
			...
			if(@op)
				n=(@op.receipt_no.slice!(3..50).to_i+1).to_s
			else
				n=1.to_s
			end
			str="ISS"+n
			...
			if(@issues_to_op.patient_type=="OP" || @issues_to_op.patient_type=="OSP"|| @issues_to_op.patient_type=="IP" || params[:form_name].to_s=="arogya")
				for store in @issues_to_op.issue_op_child
					@store_child=StoreChild.all(:conditions => "item_name = '#{store.item_name}' and quantity > 0")
			...
			if(@issues_to_op.patient_type.to_s=="IP" && @issues_to_op.reg_type.to_s=="Arogyasree" && params[:form_name].to_s=="arogya")
				...
				format.html { redirect_to("/issues_to_ops/report1/#{@issues_to_op.id}?user_id=#{user_id}&format=pdf") }
			end
...
@discount=  @issues_to_op.total_amount.to_f - @issues_to_op.due_amount.to_f
  ...
  def update
	...
	for sales in  @issues_to_op.issue_op_child
        ...
        format.html { redirect_to("/issues_to_ops/report1/#{@issues_to_op.id}?format=pdf", :notice => 'IssuesToOp was successfully updated.') }
        format.xml  { head :ok }
      else
  ...
  def ajax_building_arogyasree
   ...
   def day_wise_report
	 ...
	 def observe_field_ex
  	...
  	@issues_to_op = IssuesToOp.find(:all,:conditions=>"issue_no LIKE '%#{params[:issue_no]}' AND issue_date LIKE '#{params[:issue_date]}%' AND due_amount LIKE '#{params[:due_amount]}%' AND due LIKE '#{params[:due]}%' AND paid_amt LIKE '#{params[:paid_amt]}%'")
		if( @issues_to_op[0])
		   render:partial=>"search_op_record"
		else
