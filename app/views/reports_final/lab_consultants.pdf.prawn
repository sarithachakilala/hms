...
i=0
k=1
	for testbooking in @testbooking
	pdf.stroke do
	if(page_no==0)
			...
			if !@test.empty? || !@department.empty?
				...
				if(@child[0])
					for testname in @child 
						...
						pdf.draw_text "#{testbooking.mr_no}", :at => [45,735-10*i], :size =>9
						...
						if(testbooking.booking_date!="")
							pdf.draw_text "#{testbooking.created_at.strftime('%d-%m-%Y  %H:%M')}", :at => [285,735-10*i], :size =>9
						end
						pdf.draw_text "#{testname.test_name}", :at => [365,735-10*i], :size =>9
						pdf.draw_text "#{testbooking.refferal_doctor}", :at => [455,735-10*i], :size =>9
						pdf.draw_text "#{testname.amount}", :at => [520,735-10*i], :size =>9
				...
				for testname in testbooking.test_booking_child 
					...
					pdf.draw_text "#{testname.test_name}", :at => [365,735-10*i], :size =>9
					pdf.draw_text "#{testbooking.refferal_doctor}", :at => [455,735-10*i], :size =>9
					pdf.draw_text "#{number_with_precision(testname.amount, :precision => 2, :separator =>'.')}", :at => [520,735-10*i], :size =>9
					i=i+1
					total=total+testname.amount
				end
			end
